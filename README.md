...
├── map_cache/               # 离线瓦片金字塔缓存池 (Git 已忽略)
└── README.md                # 项目说明文档
---
🛠️ 快速开始 (Quick Start)
1. 环境依赖
请确保本地已安装 Python 3.8 或更高版本。进入项目目录并安装核心依赖库：

Bash
pip install pandas geopandas matplotlib shapely contextily mercantile customtkinter openpyxl
2. 启动系统
在终端中运行主程序即可唤醒可视化控制面板：

Bash
python main.py
3. 操作流
基础设置：选择包含经纬度数据的业务 Excel 表格，设置目标保存目录与渲染质量。

视觉与算法：微调红黄灵敏度，开启/关闭需要的视觉图层（如行政区划、地铁等）。

底图与网络：选择心仪的底图风格，点击雷达面板的**“获取/补全图源”**将底图缓存至本地硬盘。

一键执行：点击左下角 🚀 开始执行空间渲染，等待进度条完成，即可在目标文件夹获取高清成图。

🔒 隐私与安全
本系统承诺：完全本地化运行。用户的业务坐标数据（Excel）、生图结果均在本地内存中处理与销毁，绝不触网上传，最高级别保障商业数据的安全与隐私。
